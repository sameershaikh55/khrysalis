                            ...
                            <SwiperSlide className='!bg-transparent relative' key={index}>
                                <a href={val?.link} className='flex flex-col gap-y-1'>
                                    <div className='relative'>
                                        {!props?.liveNews && <div className="w-full h-full flex justify-center items-center absolute">
                                        ...
                                        <img src={val?.img} className={` ${props.blips ? "!w-[270px] !h-[450px] !shadow-xl z-0 drop-shadow-2xl opacity-80" : "!w-[232px] !h-[135px]"} !w-[332px] !h-[190px] rounded-lg`} alt="" />
                                    </div>
                                    {!props?.blips && <div className='flex flex-col gap-y-2'>
                                        <div className="flex items-center gap-x-2">
                                            <span className='text-[13px] font-bold'>{val?.title}</span>
                                        </div>
                                        {props?.details && <div className='flex flex-col gap-y-1'>
                                            <div className='flex items-center text-sm ml-6 gap-x-1'>
                                                <img src={adminIcon} alt="" className='w-3'/>
                                                <span className='text-[10px] font-normal'>{val?.userName}</span>
                                            </div>
                                            <div className='flex items-center text-sm ml-6 gap-x-1'>
                                                <img src={calendarImg} alt="" className='relative bottom-[1px]'/>
                                                <span className='text-[10px] font-normal'>{val?.timeAgo}</span>
                                            </div>
                                        </div>}
                                    </div>}
                                    {
                                        props?.blips && <div className='flex items-start gap-x-2'>
                                            <img src={profileBlips} alt="" className='w-12' />
                                            <div className='flex gap-y-2 flex-col'>
                                                <span className='text-[13px] font-bold'>{val?.title}</span>
                                                <div>
                                                    <span className='block font-thin text-xs'>{val?.userName}</span>
                                                    <span className='block font-thin text-xs'>{val?.timeAgo}</span>
                                                </div>
                                            </div>
                                        </div>
                                    }
